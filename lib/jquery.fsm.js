...
(function($) {
  ...
  $.fn.fsm = function(options) {
	...
	$.fsm_event = function(machine, event_name, from_states, to_state, conditions) { 
	  ...
	  // fire
	  this.fire = function() {  
	    if( transition = transition_for(this) ) transition.perform();
	  };
	  ...
	  this.transition_for = function( requirements ){
	    if( !requirements['from'] ) requirements['from'] = this.machine.states[this.event_name].event_name;
	    
	    for( i in this.guards ) {
	      guard = this.guards[i];
	      if( match = guard.match(this, requirements) ) {
	        from = requirements['from'];
	        to = match['to'].values.len ? match['to'].values[0] : from;
	        
	        return $.fsm_transition( this, this.machine, this.event_name, from, to );
	      }
	    }
